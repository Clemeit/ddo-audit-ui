...
import { RaceDemographicApiData } from "../../models/Demographics.ts"
import { RangeEnum, ServerFilterEnum } from "../../models/Common.ts"
import { getRaceDemographic } from "../../services/demographicsService.ts"
...
const RacePopulationDistribution = () => {
    const [dataMap, setDataMap] = useState<
        Record<RangeEnum, RaceDemographicApiData | undefined>
    >({
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                setIsLoading(true)
                const retrievedData = await getRaceDemographic(
                    rangeFilter,
                    controller.signal
                )
    ...
    const nivoData: { data: NivoPieSlice[]; total: number } = useMemo(() => {
        ...
        Object.entries(dataMap[rangeFilter]).forEach(
            ([serverName, serverData]) => {
                ...
                Object.entries(serverData).forEach(([race, count]) => {
                    if (race.toLowerCase().includes("unknown")) return
                    const series = nivoData.find((s) => s.id === race)
                    total += count ?? 0
                    if (series) {
