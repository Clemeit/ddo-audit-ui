...
import { GuildAffiliatedDemographicApiData } from "../../models/Demographics.ts"
import { RangeEnum, ServerFilterEnum } from "../../models/Common.ts"
import { getGuildAffiliatedDemographic } from "../../services/demographicsService.ts"
...
const GuildAffiliatedPopulationDistribution = () => {
    const [dataMap, setDataMap] = useState<
        Record<RangeEnum, GuildAffiliatedDemographicApiData | undefined>
    >({
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                setIsLoading(true)
                const retrievedData = await getGuildAffiliatedDemographic(
                    rangeFilter,
                    controller.signal
                )
        ...
        Object.entries(dataMap[rangeFilter]).forEach(
            ([serverName, serverData]) => {
                ...
                Object.entries(serverData).forEach(
                    ([guildAffiliationLabel, count]) => {
                        const series = nivoData.find(
                            (s) => s.id === guildAffiliationLabel
                        )
                        total += count ?? 0
                        if (series) {
                            ...
                            nivoData.push({
                                id: guildAffiliationLabel,
                                label:
                                    guildAffiliationLabel === "in_guild"
                                        ? "In Guild"
                                        : "Not in Guild",
                                value: count ?? 0,
                            })
        ...
        <div>
            ...
            <div style={{ height: "500px" }}>
                <ResponsivePie
                    ...
                    colors={({ id }) => {
                        if (id.toString() === "in_guild")
                            return "rgba(255, 142, 76, 1)"
                        return "#888"
                    }}
