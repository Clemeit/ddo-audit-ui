...
import { PrimaryClassDemographicApiData } from "../../models/Demographics.ts"
import { RangeEnum, ServerFilterEnum } from "../../models/Common.ts"
import { getPrimaryClassDemographic } from "../../services/demographicsService.ts"
...
const PrimaryClassPopulationDistribution = () => {
    const [dataMap, setDataMap] = useState<
        Record<RangeEnum, PrimaryClassDemographicApiData | undefined>
    >({
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                setIsLoading(true)
                const retrievedData = await getPrimaryClassDemographic(
                    rangeFilter,
                    controller.signal
                )
    ...
    const nivoData: { data: NivoPieSlice[]; total: number } = useMemo(() => {
        ...
        Object.entries(dataMap[rangeFilter]).forEach(
            ([serverName, serverData]) => {
                ...
                Object.entries(serverData).forEach(([primaryClass, count]) => {
                    if (primaryClass.toLowerCase() === "none") return
                    const series = nivoData.find((s) => s.id === primaryClass)
                    total += count ?? 0
                    if (series) {
